...
<%-- 				${computerListManager.numberOfElementsFromFilteredRequest} --%>
				 Computers found</h1>
	...
	<footer class="navbar-fixed-bottom">
		<div class="container text-center">
			<div class="btn-group btn-group-sm" role="group">
			         <form id="currentPageForm" action="currentPage" method="POST" class="form-inline">
                    ...
                    <c:forEach  var="i" 
                                ...
                                end="${(currentPage + 2 < maxPage) ? (currentPage + 2) : maxPage + 1}">
                          <button name="currentPageButton" type="submit" value="${i - 1}" class="btn btn-default ${currentPage == i - 1? 'active' : ''}">${i}</button>
                  </c:forEach>
                    
                    <c:if test="${currentPage < maxPage - 1}">
                        <button name="currentPageButton" type="submit" value="${maxPage}" class="btn btn-default" aria-label="Next"> <span aria-hidden="true">&raquo;</span></button>
                    </c:if>
